...
class Paul(Actor):
    ...
    def get_open_adj_coords(self, coords):
        ...
        diag_all_adj = [self.world._add_coords(coords, d) for d in diag_directions]
        diag_in_bounds = [self.is_valid(c) for c in diag_all_adj]
        for i, coord in enumerate(diag_all_adj):
            if diag_in_bounds[i]:
                costs.append(3 + self.world.get_terrain_cost(coord))
                open_adj.append(coord)

        #Hops
        hop_dirs = [(0, 2), (2, 0), (0, -2), (-2, 0)]
        hop_all_adj = [self.world._add_coords(coords, d) for d in hop_dirs]
        hop_in_bounds = [self.is_valid(c) for c in hop_all_adj]
        for i, coord in enumerate(hop_all_adj):
            if hop_in_bounds[i]:
